...
- `control-token`：本机控制令牌；首次安装默认值为 `1`，也可通过安装器“重新生成令牌”更换为随机值。
...
面板的“监控插件”区域提供 `共享浏览器运行环境`。首次部署后可一键安装 `Chromium`、`Noto CJK 中文字体`、`Xvfb`、`x11vnc`、`noVNC/websockify` 与运行依赖，供浏览器会话、网页交互、自动化任务及需要登录态的功能共用；MonkeyCode 登录和 Cookie 同步只是其中一种用途。组件通过当前 VM 已配置的软件源安装，使用系统 apt 源，不依赖 GitHub、npm 或固定镜像地址；所需软件源应由该 VM 访问。

PC 和手机均可通过保活中心打开受控浏览器；需要登录 MonkeyCode 时，可在浏览器中完成登录后由 jiankong 读取浏览器 Cookie，并通过 HTTPS 同步到 Worker、用真实 Task WebSocket 验证。浏览器 profile 保存在 `data/monkeycode-login-browser/profile/`；Cookie 不在网页、日志或 Git 中显示。

- noVNC 连接密码固定为：`1`。
- jiankong 控制令牌仍只保护面板 API 和“打开受控浏览器”动作；它不再作为 VNC 密码。
